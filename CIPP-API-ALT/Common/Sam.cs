        ...
        public static async Task<JsonElement> CreateSAMAuthApp(string appName, SamAppType appType, string domain, string swaggerUiAppId="", string spaRedirectUri="", string scopeGuid="")
        {
            ...
            switch (appType)
            {
                case SamAppType.Api:
					...
					var appPassword = await RequestHelper.NewGraphPostRequest(string.Format("https://graph.microsoft.com/v1.0/applications/{0}/addPassword", createdSamApp.GetProperty("id").GetString()), ApiEnvironment.Secrets.TenantId, new PasswordCredential() { displayName="CIPP-Pwd" }, HttpMethod.Post, "https://graph.microsoft.com/Application.ReadWrite.All", false);

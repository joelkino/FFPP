...
namespace FFPP.Api.v10.Users
{
    ...
    public class User
    {
        ...
        public static async Task<List<User>> GetUsers(string accessingUser = "", string tenantFilter = "", string userId = "")
        {
            ...
            if (!string.IsNullOrEmpty(userId))
            {
                ...
                if (responseMessage.IsSuccessStatusCode)
                {
                    ...
                    {
                        FfppLogs.DebugConsoleWrite("Did not get HTTP 200 success attempting to fetch user last signin details.");
                    }
                }
                else
                {
                    FfppLogs.DebugConsoleWrite("Did not get HTTP 200 success attempting to fetch token for user last signin details.");
                }
            }
        ...
        public async static Task<List<OneDriveSiteReport>> GetSites(string tenantFilter, string type, string userUpn, string accessingUser = "")
        {
            ...
            {
                FfppLogs.DebugConsoleWrite(string.Format("Couldn't delete cached file {0}", filename));
            }
            return outRes;
        }
        ...
        public static async Task<List<UserDevice>> GetUserDevices(string tenantFilter, string userId, string accessingUser = "")
        {
            ...
            {
                FfppLogs.DebugConsoleWrite(string.Format("Exception in ListUserDevices: {0} Inner Exception: {1}", ex.Message, ex.InnerException.Message ?? string.Empty));
                throw;
            }

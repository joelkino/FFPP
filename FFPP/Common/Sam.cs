...
namespace FFPP.Common
{
    public class Sam
    {
        ...
        public static async Task<JsonElement> CreateSAMAuthApp(string appName, SamAppType appType, string domain, string swaggerUiAppId="", string[]? spaRedirectUri= null, string scopeGuid="")
        {
            dynamic samApp;
            ...
            switch (appType)
            {
                case SamAppType.Api:
					...
					await Task.Delay(10000); // Have to wait about 10 seconds for Azure to properly replicate the app before we can set password on it
					var appPassword = await RequestHelper.NewGraphPostRequest(string.Format("https://graph.microsoft.com/v1.0/applications/{0}/addPassword", createdSamApp.GetProperty("id").GetString()), ApiEnvironment.Secrets.TenantId, new PasswordCredential() { displayName="FFPP-Pwd" }, HttpMethod.Post, "https://graph.microsoft.com/Application.ReadWrite.All", false);
					return createdSamApp;
